...
public class ImportBPartnerRSGE extends SvrProcess {
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (impBP.getC_BPartner_Location_ID() != 0)		
				{
					...
					location.setXX_District_ID(impBP.getXX_District_ID());
					location.setXX_SubDistrict_ID(impBP.getXX_SubDistrict_ID());
				}
							 ...
							 Map<Integer,MUser> userMapForImpBP) throws Exception
	{
		...
		for(ImportBPartnerModel impBP : impBPartner)
		{
		...
		for(ImportBPartnerModel imp : impBPartner)
		{
