 ...
 * 
 */
public class GenPOFromRequisitionNew extends SvrProcess {
...
//		errorMsg = OrgUtils.checkOrgFunction(getCtx(), p_AD_Org_ID,
//				getAD_Client_ID(), OrgUtils.trxType_Purchase, get_Trx());
//		if (errorMsg != null)
//			return errorMsg;
//
		// Specific
		...
		if (p_AD_Org_ID != 0)
			sql.append(" AND r.PurchasingOrganization_ID=?");
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID)
		...
		if (requisition.get_ValueAsBoolean("IsRawMaterial"))
			m_order.set_Value("IsRawMaterial", true);
		if (requisition.get_ValueAsBoolean("IsPackaging"))
			m_order.set_Value("IsPackaging", true);
