	...
	protected String doIt() throws Exception
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				/**
				 *  Convert grand total and open amount to default accounting schema's currency if :
				 *  1 - No Currency selected
				 *  2 - Invoice's currency is different than default accounting schema's currency
				 *  
				 *  By Fanny Rahmadhitya (RMM) - 2016
				 */
				if(p_C_Currency_ID==0)
				{
					MClientInfo ci = MClientInfo.get(getCtx(), getAD_Client_ID(), get_Trx());
					MAcctSchema as = new MAcctSchema(getCtx(), ci.getC_AcctSchema1_ID(), get_Trx());
					
					if(as.getC_AcctSchema_ID()!=C_Currency_ID)
					{
						MInvoice inv = new MInvoice(getCtx(), C_Invoice_ID, get_Trx());
						GrandTotal = MConversionRate.convert(getCtx(), GrandTotal, C_Currency_ID, as.getC_Currency_ID(), getAD_Client_ID(), inv.getAD_Org_ID());
						OpenAmt = MConversionRate.convert(getCtx(), OpenAmt, C_Currency_ID, as.getC_Currency_ID(), getAD_Client_ID(), inv.getAD_Org_ID());						
					}
				}
