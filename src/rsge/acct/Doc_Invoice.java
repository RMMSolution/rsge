 ...
 *  </pre>
 *  @author Jorg Janke
 *  @version  $Id: Doc_Invoice.java 9977 2011-12-03 03:34:51Z ssharma $
 *  @version  $Id: Doc_Invoice.java RSGE 2014-07-06 Fanny Rahmadhitya
 */
public class Doc_Invoice extends Doc
{
	...
	private int				sourceOrgID = 0;
	
	private BigDecimal		invoiceDiscAmt = BigDecimal.ZERO;
	...
	public String loadDocumentDetails()
	{
		...
		MOrder order = new MOrder(getCtx(), invoice.getC_Order_ID(), getTrx());
		if(order!=null)
			sourceOrgID = order.getAD_Org_ID();
		...
		// Get Service Charge
		if(invoice.isSOTrx())
			serviceChargeAmt = invoice.getServiceChargeAmt();
		// Get Invoice Discount
		invoiceDiscAmt = invoice.getInvoiceDiscAmt();
	...
	private DocLine[] loadLines (MInvoice invoice)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			docLine.setAmount (LineNetAmt, PriceList, Qty,StdPrecision);	//	qty for discount calc
			if (docLine.isItem())
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
			...
			|| getDocumentType().equals(MDocBaseType.DOCBASETYPE_ARProFormaInvoice))
		{
			...
			// Service Charge
			if(serviceChargeAmt.signum()!=0)
			{
				fact.createLine(null, DocBaseType.getAccount(DocBaseType.ACCTTYPE_ServiceCharge, 0, 0, as),
						getC_Currency_ID(), null, serviceChargeAmt);
				
			}
						
			if(invoiceDiscAmt.signum()!=0)
			{
				fact.createLine(null, DocBaseType.getAccount(DocBaseType.ACCTTYPE_InvoiceDiscount, 0, 0, as),
						getC_Currency_ID(), invoiceDiscAmt, null);
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (as.isTradeDiscountPosted())
				{
					...
					// Additional code - Use organization of source document (Sales Order, Purchase Order, etc) 					
					if(sourceOrgID!=0)
					{
						FactLine fl = fact.createLine (p_lines[i],
								p_lines[i].getAccount(ProductCost.ACCTTYPE_P_Revenue, as),
								getC_Currency_ID(), BigDecimal.ZERO, amt);
						fl.setAD_Org_ID(sourceOrgID);
						fl.save();
						if(dAmt!=null)
						{
							FactLine fl2 = fact.createLine (p_lines[i],
									p_lines[i].getAccount(ProductCost.ACCTTYPE_P_TDiscountGrant, as),
									getC_Currency_ID(), dAmt, BigDecimal.ZERO);
							fl2.setAD_Org_ID(sourceOrgID);
							fl2.save();
						}
		...
		else if (getDocumentType().equals(MDocBaseType.DOCBASETYPE_ARCreditMemo))
		{
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				// Use Return Account (RSGE)
				MAccount acct = null;
				MProductAcct pa = MProductAcct.get(p_lines[i].getM_Product_ID(), as);				
				if(pa.isUseReturnAccount())
				{
					acct = DocBaseType.getAccount(DocBaseType.ACCTTYPE_SalesReturn, p_lines[i].getM_Product_ID(), 0, as);
				}
				else
				{
				...
				if (as.isTradeDiscountPosted())
				{
					...
					// Modify by Fanny R - 06/06/2014
					if(sourceOrgID!=0)
					{
						FactLine fl = fact.createLine (p_lines[i], acct,
					...
					{
						fact.createLine (p_lines[i], acct,
								getC_Currency_ID(), amt, BigDecimal.ZERO);
						if(dAmt!=null)
						fact.createLine (p_lines[i],
								p_lines[i].getAccount (ProductCost.ACCTTYPE_P_TDiscountGrant, as),
			...
			MAcctSchemaDefault defaultAcct = MAcctSchemaDefault.get(getCtx(), as.getC_AcctSchema_ID());
			if(defaultAcct.isUseReturnAccount())
			{
				receivables_ID = DocBaseType.getValidCombination_ID(DocBaseType.ACCTTYPE_UnallocatedReturn, 0, 0, as);
				receivablesServices_ID = receivables_ID;
			}
			else
			{
			...
			System.out.println("Gross Amt " + grossAmt);
			if (grossAmt.signum() != 0)
				fact.createLine(null, MAccount.get(getCtx(), receivables_ID),getC_Currency_ID(), null, grossAmt.subtract(totalDisc));
			if (serviceAmt.signum() != 0)
				fact.createLine(null, MAccount.get(getCtx(), receivablesServices_ID),getC_Currency_ID(), null, serviceAmt.subtract(totalDisc));
		}
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (!landedCost)
				{
					...
					if (as.isTradeDiscountPosted())
					{
						...
						if(dAmt!=null)
							fact.createLine (line, line.getAccount(ProductCost.ACCTTYPE_P_TDiscountRec,as),getC_Currency_ID(), BigDecimal.ZERO, dAmt);
						fact.createLine (line,expense ,getC_Currency_ID(), amt, BigDecimal.ZERO);
					}
					else					
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (!landedCost)
				{
					...
					if (as.isTradeDiscountPosted())
					{
						...
						if(dAmt!=null)
						fact.createLine (line, line.getAccount(ProductCost.ACCTTYPE_P_TDiscountRec,as),getC_Currency_ID(), dAmt,BigDecimal.ZERO);
						fact.createLine (line,expense ,getC_Currency_ID(),BigDecimal.ZERO,amt);
					}
					else					
						fact.createLine (line,expense,getC_Currency_ID(), dAmt, amt);
