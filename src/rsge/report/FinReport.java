...
import rsge.model.MAnnualFinRecord;
import rsge.report.MReport;
import rsge.report.MReportColumn;
import rsge.report.MReportLine;
import rsge.report.MReportTree;
...
public class FinReport extends SvrProcess
{
	...
	private MAnnualFinRecord		finRecord = null;
	private StringBuilder			updatedColumn = null;
	/**
	...
	protected void prepare()
	{
		...
		if(m_report.isUpdateAnnualRecord())
		{
			// Check if column set has recorded column or not
			String sql = "SELECT IsRecordedColumn FROM PA_ReportColumn " +
					"WHERE PA_ReportColumnSet_ID = ? " +
					"ORDER BY SeqNo ";
			PreparedStatement pstmt = DB.prepareStatement(sql, get_Trx());
			ResultSet rs = null;
			try{
				pstmt.setInt(1, m_report.getPA_ReportColumnSet_ID());
				...
				while(rs.next())
				{
					...
					{
						updatedColumn = new StringBuilder("Col_");
						updatedColumn.append(no);
						finRecord = MAnnualFinRecord.get(getCtx(), m_report.getC_AcctSchema_ID(), m_report.getC_Year_ID(), null, 0, get_Trx());						
						break;
					}
					no++;
				}
	...
	{System.out.println("OK START");
		...
		//	for all lines
		System.out.println("Lines " + m_lines.length);
	...
	private void updateLine (int line)
	{
		...
		for (int col = 0; col < m_columns.length; col++)
		{
			...
			// Edit by Fanny
			if(m_lines[line].isNegativeValue())
				select.append("*-1");
			...
			System.out.println("Amount Type " + m_columns[col].getAmountType());
			if (m_lines[line].getAmountType() != null)			//	line amount type overwrites column
			{
			...
			else if (m_columns[col].getAmountType() != null)
			{
				...
				else if (m_columns[col].isYear())
				{
					...
					String sql = null;
					if(!m_columns[col].isEndOfYear())
						sql = frp.getYearWhere();
					else
						sql = getYearWhere(frp);
					info.append("Year");
					select.append(sql);
				}
...
//					sb.append ("COALESCE(r2.Col_").append (c).append(",0.000000001)");
				...
				else if (m_columns[c].isColumnTypeCalculation() && m_columns[c].isCalculationTypePercent())
				{
					...
					// Edit by Fanny
					.append(",0)=0 THEN NULL ")					
					.append ("WHEN ROUND(COALESCE(Col_").append(ii_1)
					// End edit
					.append(",0),0)=0 THEN NULL ELSE ")
	...
	{
		String sql = "SELECT PA_ReportLine_ID, FinancialSummaryType FROM PA_ReportLine " +
				"WHERE PA_ReportLineSet_ID = ? " +
				"AND FinancialSummaryType IS NOT NULL ";
		PreparedStatement pstmt = DB.prepareStatement(sql, get_Trx());
		ResultSet rs = null;
		try{
			pstmt.setInt(1, m_report.getPA_ReportLineSet_ID());
			rs = pstmt.executeQuery();
			while(rs.next())
			{
				BigDecimal amt = getCalculatedAmt(rs.getInt(1));
				String summaryType = rs.getString(2);
				if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_CurrentAsset))
					finRecord.setCurrentAssetAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_CurrentLiability))
					finRecord.setCurrentLiabilityAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_COGM))
					finRecord.setTotalCOGMAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_COGS))
					finRecord.setTotalCOGSAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_EAT))
					finRecord.setEATAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_EBIT))
					finRecord.setEBITAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_EBITDA))
					finRecord.setEBITDAAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_GrossProfit))
					finRecord.setGrossProfitAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_OperatingProfit))
					finRecord.setOperatingProfitAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_TotalAsset))
					finRecord.setTotalAssetAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_TotalEquity))
					finRecord.setTotalEquityAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_TotalExpense))
					finRecord.setTotalExpenseAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_TotalLiability))
					finRecord.setTotalLiabilityAmt(amt);
				else if(summaryType.equals(MReportLine.FINANCIALSUMMARYTYPE_TotalRevenue))
					finRecord.setTotalRevenueAmt(amt);
			}
			rs.close();
			pstmt.close();
		}catch(Exception e)
	...
	private BigDecimal getCalculatedAmt(int PA_ReportLine_ID)
	{
		BigDecimal amt = BigDecimal.ZERO;
		StringBuilder sql = new StringBuilder("SELECT ");
		sql.append(updatedColumn);
		sql.append(" FROM T_Report " +
				"WHERE PA_ReportLine_ID = ? " +
				"AND AD_PInstance_ID = ? ");
...
//		}
	 ...
	 */
	public String getYearWhere (FinReportPeriod frp)
	{
		Calendar cal = Calendar.getInstance();
		cal.setTime(frp.getEndDate());
		cal.set(Calendar.DATE, 31);
		cal.set(Calendar.MONTH, Calendar.DECEMBER);
		
		StringBuffer sql = new StringBuffer ("BETWEEN ");
		sql.append(DB.TO_DATE(frp.getYearStartDate()))
			  .append(" AND ")
			  .append(DB.TO_DATE(new Timestamp(cal.getTimeInMillis())));
		return sql.toString();
	}	//	getPeriodWhere
	 ...
	 * 	@return <= end
	 */
	public String getTotalWhere (FinReportPeriod frp)
	{
		Calendar cal = Calendar.getInstance();
		cal.setTime(frp.getEndDate());
		cal.set(Calendar.DATE, 31);
		...
		sql.append(DB.TO_DATE(new Timestamp(cal.getTimeInMillis())));
		return sql.toString();
	}	//	getPeriodWhere
