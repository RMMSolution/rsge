 ...
 */
public class MReportTree implements Serializable
{
    /** Serial */
	...
	public static MReportTree get (Ctx ctx, int PA_Hierarchy_ID, String ElementType, boolean addAccessSQL) {
		String key = "H" + PA_Hierarchy_ID + "E" + ElementType;
		MReportTree tree = (MReportTree)s_treeCache.get(key);
		if (tree != null) {
			return tree;
		}
		tree = new MReportTree (ctx, PA_Hierarchy_ID, ElementType, addAccessSQL);
		s_treeCache.put(key, tree);
		return tree;
	}	//	get
	...
	/**	Map with Tree				*/
	private static final CacheBase s_treeCache
		= new CacheBase("MReportTree", CacheType.MEDIUM);
	...
	public String getWhereClause (int ID)
	{
		...
		// Edit by Fanny
		// To accomodate User Element
		String elementType = null;
		if(m_ElementType.equalsIgnoreCase("XX") || m_ElementType.equalsIgnoreCase("U1"))
			elementType = "X1";
		else if(m_ElementType.equalsIgnoreCase("U2"))
			elementType = "X2";
		else
