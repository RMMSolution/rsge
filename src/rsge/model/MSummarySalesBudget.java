...
import rsge.po.X_XX_SummarySalesBudget;

/**
 * @author bang
 * 
 */
public class MSummarySalesBudget extends X_XX_SummarySalesBudget {
	...
	private static final org.compiere.util.CLogger log = org.compiere.util.CLogger
			.getCLogger(MSummarySalesBudget.class);
	/**
	 * @param ctx
	 * @param XX_SummarySalesBudget_ID
	 * @param trx
	 */
	public MSummarySalesBudget(Ctx ctx, int XX_SummarySalesBudget_ID, Trx trx) {
	 ...
	 * @param trx
	 */
	public MSummarySalesBudget(Ctx ctx, ResultSet rs, Trx trx) {
		super(ctx, rs, trx);
		// TODO Auto-generated constructor stub
	}
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if (price.signum() == 0) {
			MProduct mProduct = new MProduct(getCtx(), getM_Product_ID(),
					get_Trx());
			log.saveError("Error", Msg.getMsg(getCtx(), "No Price for = "
					+ mProduct.getName()));
			return false;
		}
